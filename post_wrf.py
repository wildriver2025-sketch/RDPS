...
def calc_vis(qv, qc, qr, qi, qs, t, p, t2):
    '''
!
...
!
    ...
    #VIS.attrs["GRIB_paramId"] = 3020
    VIS.attrs["GRIB_discipline"] = 0
    VIS.attrs["GRIB_parameterCategory"] = 19
    VIS.attrs["GRIB_parameterNumber"] = 0

    return VIS


def calc_gust(gph, ter, pbl, u10, v10, u, v):
    ...
    GUST.attrs["description"] = "GUST"
    GUST.attrs["units"] = u10.attrs["units"]
    GUST.attrs["stagger"] = u10.attrs["stagger"]
    GUST.attrs["coordinates"] = u10.attrs["coordinates"]
    #GUST.attrs["GRIB_paramId"] = 260065
    GUST.attrs["GRIB_discipline"] = 0
    GUST.attrs["GRIB_parameterCategory"] = 2
    GUST.attrs["GRIB_parameterNumber"] = 22

    return GUST


def ConvectiveRain(rainc):
    value = rainc
    value = value.rename("convective_rain")
    value.attrs["FieldType"] = rainc.attrs["FieldType"]
    value.attrs["MemoryOrder"] = rainc.attrs["MemoryOrder"]
    value.attrs["description"] = "CONVECTIVE PRECIPITATION"
    value.attrs["units"] = rainc.attrs["units"]
    value.attrs["stagger"] = rainc.attrs["stagger"]
    value.attrs["coordinates"] = rainc.attrs["coordinates"]
    return value

# For KDM6
#def LargeScaleRain(rainnc, snow, graupel, hail):
#    value = rainnc - (snow + graupel + hail)
def LargeScaleRain(rainnc, snow, graupel):
    value = rainnc - (snow + graupel)
    value = value.rename("large_scale_rain")
    value.attrs["FieldType"] = rainnc.attrs["FieldType"]
    value.attrs["MemoryOrder"] = rainnc.attrs["MemoryOrder"]
    value.attrs["description"] = "LARGE-SCALE PRECIPITATION"
    value.attrs["units"] = rainnc.attrs["units"]
    value.attrs["stagger"] = rainnc.attrs["stagger"]
    value.attrs["coordinates"] = rainnc.attrs["coordinates"]
    return value


def ConvectiveSnow(snow):
    value = snow - snow
    value = value.rename("convective_snow")
    value.attrs["FieldType"] = snow.attrs["FieldType"]
    value.attrs["MemoryOrder"] = snow.attrs["MemoryOrder"]
    value.attrs["description"] = "CONVECTIVE SNOW"
    value.attrs["units"] = snow.attrs["units"]
    value.attrs["stagger"] = snow.attrs["stagger"]
    value.attrs["coordinates"] = snow.attrs["coordinates"]
    return value


#def LargeScaleSnow(snow, graupel, hail):
#    value = snow + graupel + hail
def LargeScaleSnow(snow, graupel):
    value = snow + graupel
    value = value.rename("large_scale_snow")
    value.attrs["FieldType"] = snow.attrs["FieldType"]
    value.attrs["MemoryOrder"] = snow.attrs["MemoryOrder"]
    value.attrs["description"] = "LARGE-SCALE SNOW"
    value.attrs["units"] = snow.attrs["units"]
    value.attrs["stagger"] = snow.attrs["stagger"]
    value.attrs["coordinates"] = snow.attrs["coordinates"]
    return value
